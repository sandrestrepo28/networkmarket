...
processed_results = []
target_count = 5  # Queremos 5 resultados
should_stop = threading.Event()  # Señal para detener todo cuando tengamos suficientes
...
def is_account_private(username):
    """Verifica si una cuenta es privada usando la API GraphQL"""
    try:
...
def get_user_info_safely(user_id, username):
    """Función mejorada para obtener info de usuario con múltiples estrategias"""
    try:
...
def process_follower_complete(follower_id, follower, target_username):
    """Procesa un seguidor completo en un solo hilo - incluye verificación y procesamiento"""
    global processed_results
    
    # Si ya tenemos suficientes resultados, detener
    ...
    try:
        ...
        # 1. Verificar si es privada
        is_private = is_account_private(follower.username)
        
        # Si es privada, terminamos
        ...
        # 3. Obtener la fecha del último post
        ...
        with result_lock:
            ...
            # Si llegamos al objetivo, señalizar para detener
...
def main():
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            ...
            # Esto solo es para mostrar progreso
            for i, future in enumerate(concurrent.futures.as_completed(futures)):
                # Si ya llegamos al objetivo, cancelar los futuros pendientes
                if should_stop.is_set():
                    for f in futures:
                        if not f.done():
                            f.cancel()
